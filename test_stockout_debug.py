...
for year_offset, method_list in enumerate(sim_stock.results['method_usage']):
    ...
    if year >= 2025:
        print(f"Year {year}: Method 1 = {method_list[1]:.3f}, Method 3 = {method_list[3]:.3f}")


# --- CPR plot comparison ---
plt.figure(figsize=(10, 5))
plt.plot(sim_base.results['t'], sim_base.results['cpr'] * 100, label='No stockout')  # CPR × 100 for %
plt.plot(sim_stock.results['t'], sim_stock.results['cpr'] * 100, label='With 100% stockout (Pills & Injectables)', linestyle='--')
plt.axvspan(2025, 2030, color='gray', alpha=0.2, label='Stockout period')
plt.xlabel('Year')
plt.ylabel('CPR (%)')
plt.title('Kenya: Impact of Pill & Injectable Stockout on CPR (one simulation, 1000 agents at the start)')
plt.ylim(0, 60)
plt.legend()
plt.grid(True)
plt.tight_layout()
